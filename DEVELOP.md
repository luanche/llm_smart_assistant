...
├── __init__.py           # Entry point, services (process_input, toggle_automation, etc.), panel registration
├── manifest.json         # Dependencies and version
├── const.py              # Constants, default prompts (DEFAULT_PROMPT_DEFAULT, DEFAULT_PROMPT_AUTOMATION)
├── config_flow.py        # Single-page ConfigFlow + OptionsFlow (24 fields)
├── coordinator.py        # Core: LLM API calls, ReAct reasoning loop, automation listener management
...
└── translations/
    ├── en.json           # HA translation keys
    └── zh-Hans.json      # Chinese translations
```

## Hot Reloading

Panel files (`panel/index.html`, `panel/chat.js`) are read from disk on every request. Edit them and refresh the browser — no HA restart needed.

For Python changes (`*.py`), restart HA:
...
```
HA Sidebar → panel_custom → chat.js (LitElement) → <iframe src="/api/llm_smart_assistant/chat_panel">
  → index.html (full UI)
```
...
1. Add an entry to the `LANGUAGES` object with all 54 keys.
...
# Save baseline for diff detection
...
Checks:
- `panel/index.html` — i18n key coverage, hardcoded strings, HTML labels
