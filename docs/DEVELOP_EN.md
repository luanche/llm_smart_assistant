...
| `schedule` planner (once/daily/weekly/monthly)   | Task 7c: `_compute_next_fire` computes the next occurrence (weekly scans 7 days, monthly scans 62 days across month boundaries, once fires at an exact datetime then never repeats); `_register_time_trigger` re-registers per schedule and stops when there is no future time; the LLM can create these via natural language ("every Monday…" → weekly weekdays:[1]) |
