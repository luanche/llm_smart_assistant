...
- 日志: `/root/projects/hass-openai-agent/config/home-assistant.log`
- 存储: `/root/projects/hass-openai-agent/config/.storage/llm_smart_assistant.storage`
...
python3 << 'PYEOF'
import json
path = '/root/projects/hass-openai-agent/config/.storage/llm_smart_assistant.storage'
...
```bash
# 查看 LLM 原始响应
grep "LLM raw response" /root/projects/hass-openai-agent/config/home-assistant.log | tail -5

# 查看解析后的 JSON
grep "LLM JSON parsed" /root/projects/hass-openai-agent/config/home-assistant.log | tail -5

# 查看推理轮次
grep "Reasoning round\|completed" /root/projects/hass-openai-agent/config/home-assistant.log | tail -10

# 查看步骤执行
grep "Step execution\|Executed service" /root/projects/hass-openai-agent/config/home-assistant.log | tail -10
```
...
检查 `const.py` 中 `RESTRICTED_DOMAINS` 列表。
`input_number`、`input_select` 默认被限制，测试时需要移除。
...
```python
import json
d = json.load(open('/root/projects/hass-openai-agent/config/.storage/llm_smart_assistant.storage'))
d['data']['history'] = []
