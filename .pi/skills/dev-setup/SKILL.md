...
All local credentials live in the gitignored **`.user/`** directory (project root):

| File | Content | Written by |
|------|---------|-----------|
| `.user/hass_token` | Long-lived access token (plain text, mode 600) | `setup_env.py` (or manually) |
| `.user/credentials.json` | `ha_url`, `ha_username`, `ha_password`, `llm_base_url`, `llm_api_key`, `llm_model` | `setup_env.py` after a successful integration setup |
...
| 4. Token | Validate cached `.user/hass_token`; otherwise mint a 10-year long-lived token via WebSocket | ✅ reuses valid token |
...
- **Token invalid** — delete `.user/hass_token` and re-run; the script mints a new one (requires onboarding token or a provided `--ha-token`).
