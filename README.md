...
**定时计划** — 时间触发支持 4 种 schedule（UI 表单或自然语言均可）：

| 计划 | 示例 | 说明 |
|------|------|------|
| 一次性 | "8月15日下午2点打开电视" | 指定年月日时分（支持秒），触发后不重复 |
| 每天 | "每天晚上11点关闭空调" | 每天固定时刻 |
| 每周 | "每个星期一早上8点启动扫地机器人" | 可选多天（一~日），如每周一、三、五 |
| 每月 | "每月1号上午9点打开车库门" | 可选多日（1~31），如每月1号和15号 |

**一次性自动化** — 添加 `one_shot`，触发一次后自动删除（适合"1分钟后关闭空调"）。

**定时提醒** — 直接说"1分钟后提醒我出门"、"每天早上8点提醒我吃药"等，到点后自动 TTS 播报提醒（如"该出门了，记得带钥匙"）；一次性或周期均可。
